...
class MappingMortar(Mapping):
    ...
    def __init__(self, maps, helper_to_orig, helper_maps, super_to_helper, supermap, normals):
    ...
    @classmethod
    def init_2D(cls, mesh1: Mesh2D, mesh2: Mesh2D, boundary1: ndarray, boundary2: ndarray, tangent: ndarray):
        ...
        m1 = MeshLine(np.sort(param_p1), np.array([np.arange(p1.shape[1] - 1), np.arange(1, p1.shape[1])]))
        m2 = MeshLine(np.sort(param_p2), np.array([np.arange(p2.shape[1] - 1), np.arange(1, p2.shape[1])]))
        ...
        f1mps = 0.5 * (mesh1.p[:, mesh1.facets[0, boundary1]] + mesh1.p[:, mesh1.facets[1, boundary1]])
        sort_boundary1 = np.argsort(param(f1mps))
        z1 = map_mesh1.G(map_m1.invF(map_super.F(np.array([[0.25, 0.75]])), tind=ix1), find=boundary1[sort_boundary1][ix1])
        ix1_flip = np.unique(ix1[param(z1[:, :, 1]) < param(z1[:, :, 0])])
        m1t = m1.t.copy()
        m1t[:, ix1_flip] = np.flipud(m1t[:, ix1_flip])
        m1 = replace(m1, t=m1t)
        f2mps = 0.5 * (mesh2.p[:, mesh2.facets[0, boundary2]] + mesh2.p[:, mesh2.facets[1, boundary2]])
        sort_boundary2 = np.argsort(param(f2mps))
        z2 = map_mesh2.G(map_m2.invF(map_super.F(np.array([[0.25, 0.75]])), tind=ix2), find=boundary2[sort_boundary2][ix2])
        ...
        ix1 = np.digitize(mps[0, :, 0], m1.p[0]) - 1
        ix2 = np.digitize(mps[0, :, 0], m2.p[0]) - 1
        return cls((map_mesh1, map_mesh2), (boundary1[sort_boundary1][ix1], boundary2[sort_boundary2][ix2]), (map_m1, map_m2), (ix1, ix2), map_super, normals)
    ...
    def G(self, X: ndarray, find: ndarray=None) -> ndarray:
        side = self.side
        return self.maps[side].G(self.helper_maps[side].invF(self.supermap.F(X), tind=self.super_to_helper[side]), find=self.helper_to_orig[side])

    def detDG(self, X: ndarray, find: ndarray=None) -> ndarray:
        if X.shape[0] == 1:
            segs = self.G(np.array([[0.0, 1.0]]))
            return np.sqrt(np.diff(segs[0], axis=1) ** 2 + np.diff(segs[1], axis=1) ** 2)
        raise NotImplementedError

    def normals(self, X: ndarray, tind: ndarray, find: ndarray, t2f: ndarray) -> ndarray:
        return np.repeat(self._normals[:, :, None], X.shape[-1], axis=2)