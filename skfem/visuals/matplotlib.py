...
def draw_meshtet(m: MeshTet, **kwargs) -> Axes:
    ...
    ax.plot_trisurf(m.p[0], m.p[1], m.p[2], triangles=indexing, cmap=plt.cm.viridis, edgecolor='k')
    ax.set_axis_off()
    return ax
...
def draw_mesh2d(m: Mesh2D, **kwargs) -> Axes:
    ...
    xs = []
    ys = []
    for (s, t, u, v) in zip(m.p[0, facets[0]], m.p[1, facets[0]], m.p[0, facets[1]], m.p[1, facets[1]]):
        xs.append(s)
        xs.append(u)
        xs.append(None)
        ys.append(t)
    ...
    if 'facet_numbering' in kwargs:
        mx = 0.5 * (m.p[0, m.facets[0]] + m.p[0, m.facets[1]])
        my = 0.5 * (m.p[1, m.facets[0]] + m.p[1, m.facets[1]])
...
def plot_meshline(m: MeshLine, z: ndarray, **kwargs):
    ...
    for (y1, y2, s, t) in zip(z[m.t[0]], z[m.t[1]], m.p[0, m.t[0]], m.p[0, m.t[1]]):
        xs.append(s)
        xs.append(t)
        xs.append(None)
        ys.append(y1)
...
def plot_meshtri(m: MeshTri, z: ndarray, **kwargs) -> Axes:
    ...
    if 'ax' not in kwargs:
        ...
        ax = kwargs['ax']
    im = ax.tripcolor(m.p[0], m.p[1], m.t.T, z, **{k: v for (k, v) in kwargs.items() if k in ['shading', 'edgecolors', 'cmap', 'vmin', 'vmax']})
...
def plot3_meshtri(m: MeshTri, z: ndarray, **kwargs) -> Axes:
    ...
    ax.plot_trisurf(m.p[0], m.p[1], z, triangles=m.t.T, cmap=plt.cm.viridis, antialiased=False)
    return ax
