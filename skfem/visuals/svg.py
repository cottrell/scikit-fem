...
def draw_mesh2d(m: Mesh2D, **kwargs) -> SvgPlot:
    ...
    template = '<line x1="{}" y1="{}" x2="{}" y2="{}" style="stroke:{};stroke-width:{}"/>'
    lines = ''
    for (s, t, u, v) in zip(p[0, facets[0]], p[1, facets[0]], p[0, facets[1]], p[1, facets[1]]):
        lines += template.format(s, t, u, v, color, stroke)
    return SvgPlot('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="{}" height="{}">{}</svg>'.format(width, height, lines))
...
def plot_mesh2d(m: Mesh2D, x: ndarray, **kwargs) -> SvgPlot:
    ...
    maxval = np.max(x)
    (p, width, height, stroke) = points_to_figure(m.p.copy(), kwargs)
    template = '<polygon points=' + '"' + '{},{} ' * t.shape[0] + '"' + 'style="fill:rgb({}, {}, {});" />'
    elems = ''
    ...
    return SvgPlot('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="{}" height="{}" shape-rendering="crispEdges">{}</svg>'.format(width + 30, height, elems))
