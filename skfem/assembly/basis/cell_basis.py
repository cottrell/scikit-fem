...
from ...element import DiscreteField, Element
...
class CellBasis(AbstractBasis):
    ...
    def __init__(self, mesh: Mesh, elem: Element, mapping: Optional[Mapping]=None, intorder: Optional[int]=None, elements: Optional[ndarray]=None, quadrature: Optional[Tuple[ndarray, ndarray]]=None):
        ...
        """
        super(CellBasis, self).__init__(mesh, elem, mapping, intorder, quadrature, mesh.refdom)
        self.basis = [self.elem.gbasis(self.mapping, self.X, j, tind=elements) for j in range(self.Nbfun)]
        if elements is None:
            self.nelems = mesh.nelements
        else:
            self.nelems = len(elements)
        self.tind = elements
        self.dx = np.abs(self.mapping.detDF(self.X, tind=elements)) * np.tile(self.W, (self.nelems, 1))
    ...
    def mesh_parameters(self) -> DiscreteField:
        return DiscreteField(np.abs(self.mapping.detDF(self.X, self.tind)) ** (1.0 / self.mesh.dim()))

    def refinterp(self, y: ndarray, nrefs: int=1, Nrefs: Optional[int]=None) -> Tuple[Mesh, ndarray]:
    ...
    def probes(self, x: ndarray) -> coo_matrix:
        ...
        phis = np.array([self.elem.gbasis(self.mapping, pts, k, tind=cells)[0][0] for k in range(self.Nbfun)]).flatten()
        return coo_matrix((phis, (np.tile(np.arange(x.shape[1]), self.Nbfun), self.element_dofs[:, cells].flatten())), shape=(x.shape[1], self.N))
    ...
    def with_element(self, elem: Element) -> 'CellBasis':
        ...
        return type(self)(self.mesh, elem, mapping=self.mapping, quadrature=self.quadrature, elements=self.tind)