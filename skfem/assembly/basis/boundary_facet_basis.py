...
from ...element import BOUNDARY_ELEMENT_MAP, DiscreteField, Element, ElementHex0, ElementQuad0, ElementTetP0, ElementTriP0
from ...mapping import Mapping
from ...mesh import Mesh, MeshHex, MeshLine, MeshQuad, MeshTet, MeshTri
...
class BoundaryFacetBasis(AbstractBasis):
    ...
    def __init__(self, mesh: Mesh, elem: Element, mapping: Optional[Mapping]=None, intorder: Optional[int]=None, quadrature: Optional[Tuple[ndarray, ndarray]]=None, facets: Optional[ndarray]=None, _side: int=0):
        ...
        """
        super(BoundaryFacetBasis, self).__init__(mesh, elem, mapping, intorder, quadrature, mesh.brefdom)
        if facets is None:
        ...
        self.normals = DiscreteField(value=self.mapping.normals(Y0, self.mesh.f2t[0, self.find], self.find, self.mesh.t2f))
        self.nelems = len(self.find)
        self.basis = [self.elem.gbasis(self.mapping, Y, j, tind=self.tind) for j in range(self.Nbfun)]
    ...
    def default_parameters(self):
        ...
        return {'x': self.global_coordinates(), 'h': self.mesh_parameters(), 'n': self.normals}
    ...
    def mesh_parameters(self) -> DiscreteField:
        return DiscreteField(np.abs(self.mapping.detDG(self.X, self.find)) ** (1.0 / (self.mesh.dim() - 1.0)) if self.mesh.dim() != 1 else np.array([0.0]))

    def _trace_project(self, x: ndarray, elem: Element) -> ndarray:
        """Trace mesh basis projection."""
        from skfem.utils import projection
        fbasis = BoundaryFacetBasis(self.mesh, elem, facets=self.find, quadrature=(self.X, self.W))
    ...
    def trace(self, x: ndarray, projection: Callable[[ndarray], ndarray], target_elem: Optional[Element]=None) -> Tuple[CellBasis, ndarray]:
        ...
        """
        DEFAULT_TARGET = {MeshTri: ElementTriP0, MeshQuad: ElementQuad0, MeshTet: ElementTetP0, MeshHex: ElementHex0}
        ...
        target_meshcls = {MeshTri: MeshLine, MeshQuad: MeshLine, MeshTet: MeshTri, MeshHex: MeshQuad}[meshcls]
        (p, t) = self.mesh._reix(self.mesh.facets[:, self.find])
        return (CellBasis(target_meshcls(projection(p), t), elemcls()), self._trace_project(x, target_elem))

    def with_element(self, elem: Element) -> 'BoundaryFacetBasis':
        """Return a similar basis using a different element."""
        return type(self)(self.mesh, elem, mapping=self.mapping, quadrature=self.quadrature, facets=self.find)