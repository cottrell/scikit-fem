...
from ...assembly.dofs import Dofs, DofsView
from ...element import DiscreteField, Element, ElementComposite
...
class AbstractBasis:
    ...
    def __init__(self, mesh: Mesh, elem: Element, mapping: Optional[Mapping]=None, intorder: Optional[int]=None, quadrature: Optional[Tuple[ndarray, ndarray]]=None, refdom: Type[Refdom]=Refdom):
        ...
        if quadrature is not None:
            ...
            (self.X, self.W) = get_quadrature(refdom, intorder if intorder is not None else 2 * self.elem.maxdeg)
    ...
    def find_dofs(self, facets: Dict[str, ndarray]=None, skip: List[str]=None) -> Dict[str, DofsView]:
        ...
        return {k: self.dofs.get_facet_dofs(facets[k], skip_dofnames=skip) for k in facets}
    ...
    def get_dofs(self, facets: Optional[Any]=None) -> Any:
        ...
        if isinstance(facets, dict):
            ...
            def to_indices(f):
                ...
                return f
            return {k: self.dofs.get_facet_dofs(to_indices(facets[k])) for k in facets}
    ...
    def interpolate(self, w: ndarray) -> Union[DiscreteField, Tuple[DiscreteField, ...]]:
    ...
    def split_indices(self) -> List[ndarray]:
        ...
        if isinstance(self.elem, ElementComposite):
            ...
            for k in range(len(self.elem.elems)):
                e = self.elem.elems[k]
                output.append(np.concatenate((self.nodal_dofs[o[0]:o[0] + e.nodal_dofs].flatten(), self.edge_dofs[o[1]:o[1] + e.edge_dofs].flatten(), self.facet_dofs[o[2]:o[2] + e.facet_dofs].flatten(), self.interior_dofs[o[3]:o[3] + e.interior_dofs].flatten())).astype(np.int_))
    ...
    def split_bases(self) -> List['AbstractBasis']:
        ...
        if isinstance(self.elem, ElementComposite):
            return [type(self)(self.mesh, e, self.mapping, quadrature=self.quadrature) for e in self.elem.elems]
