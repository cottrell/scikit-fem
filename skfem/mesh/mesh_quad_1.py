...
class MeshQuad1(Mesh2D):
    ...
    """
    doflocs: ndarray = np.array([[0.0, 0.0], [1.0, 0.0], [1.0, 1.0], [0.0, 1.0]], dtype=np.float64).T
    ...
    def _uniform(self):
        ...
        return replace(self, doflocs=np.hstack((p, p[:, self.facets].mean(axis=1), p[:, self.t].mean(axis=1))), t=np.hstack((np.vstack((t[0], t2f[0], mid, t2f[3])), np.vstack((t2f[0], t[1], t2f[1], mid)), np.vstack((mid, t2f[1], t[2], t2f[2])), np.vstack((t2f[3], mid, t2f[2], t[3])))), _boundaries=None, _subdomains=None)
    ...
    def to_meshtri(self, x: Optional[ndarray]=None):
        ...
        subdomains = None
        if self.subdomains:
            subdomains = {k: np.concatenate((v, v + self.t.shape[1])) for (k, v) in self.subdomains.items()}
        ...
        if self.boundaries:
            ...
            for k in self.boundaries:
                ...
                boundaries[k] = np.array([next(dropwhile(lambda slot: not np.array_equal(f, slot[1]), slots))[0] for f in self.facets.T[np.sort(self.boundaries[k])]])
        if self._subdomains or self._boundaries:
            mesh = replace(mesh, _boundaries=boundaries, _subdomains=subdomains)
        if x is not None:
