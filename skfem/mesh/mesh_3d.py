...
class Mesh3D(Mesh):
    ...
    def param(self) -> float:
        return np.max(np.linalg.norm(np.diff(self.p[:, self.edges], axis=1), axis=0))
    ...
    def interior_edges(self) -> ndarray:
        ...
        return np.setdiff1d(np.arange(self.edges.shape[1], dtype=np.int64), self.boundary_edges())