...
class MeshLine1(Mesh):
    ...
    doflocs: ndarray = np.array([[0.0, 1.0]], dtype=np.float64)
    ...
    def _uniform(self):
        ...
        newt = np.empty((t.shape[0], 2 * t.shape[1]), dtype=t.dtype)
        newt[0, ::2] = t[0]
    ...
    def _adaptive(self, marked):
        ...
        return replace(self, doflocs=newp, t=newt)
    ...
    def element_finder(self, mapping=None):
        ...
        def finder(x):
            ...
            elems = np.nonzero(ix[np.digitize(xin, self.p[0, ix])][:, None] == maxt)[1]
            if len(elems) < len(x):
