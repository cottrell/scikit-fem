...
class Mesh:
    ...
    def boundary_edges(self) -> ndarray:
        ...
        boundary_edges = np.sort(np.hstack(tuple([np.vstack((self.facets[itr, facets], self.facets[(itr + 1) % self.facets.shape[0], facets])) for itr in range(self.facets.shape[0])])).T, axis=1)
        edge_candidates = np.unique(self.t2e[:, self.f2t[0, facets]])
        A = self.edges[:, edge_candidates].T
        B = boundary_edges
        dims = A.max(0) + 1
        ix = np.where(np.in1d(np.ravel_multi_index(A.T, dims), np.ravel_multi_index(B.T, dims)))[0]
        return edge_candidates[ix]

    def define_boundary(self, name: str, test: Callable[[ndarray], ndarray], boundaries_only: bool=True):
        ...
        """
        warn('Mesh.define_boundary is deprecated and will be removed in the next major release.', DeprecationWarning)
        if self._boundaries is None:
            self._boundaries = {}
        self._boundaries[name] = self.facets_satisfying(test, boundaries_only)

    def with_boundaries(self, boundaries: Dict[str, Callable[[ndarray], ndarray]], boundaries_only: bool=True):
        ...
        """
        return replace(self, _boundaries={**({} if self._boundaries is None else self._boundaries), **{name: self.facets_satisfying(test, boundaries_only) for (name, test) in boundaries.items()}})
    ...
    def with_subdomains(self, subdomains: Dict[str, Callable[[ndarray], ndarray]]):
        ...
        """
        return replace(self, _subdomains={**({} if self._subdomains is None else self._subdomains), **{name: self.elements_satisfying(test) for (name, test) in subdomains.items()}})
    ...
    def _encode_cell_data(self) -> Dict[str, List[ndarray]]:
        ...
        return {**{f'skfem:s:{name}': [np.isin(np.arange(self.t.shape[1]), subdomain).astype(int)] for (name, subdomain) in subdomains.items()}, **{f'skfem:b:{name}': [((1 << np.arange(self.t2f.shape[0])) @ np.isin(self.t2f, boundary)).astype(int)] for (name, boundary) in boundaries.items()}}
    ...
    def _decode_cell_data(self, cell_data: Dict[str, List[ndarray]]):
        ...
        for (name, data) in cell_data.items():
            ...
            if subnames[1] == 's':
                ...
                boundaries[subnames[2]] = self.t2f[(1 << np.arange(self.t2f.shape[0]))[:, None] & data[0].astype(np.int64) > 0]
    ...
    def nodes_satisfying(self, test: Callable[[ndarray], ndarray], boundaries_only: bool=False) -> ndarray:
    ...
    def facets_satisfying(self, test: Callable[[ndarray], ndarray], boundaries_only: bool=False) -> ndarray:
        ...
        """
        midp = [np.sum(self.p[itr, self.facets], axis=0) / self.facets.shape[0] for itr in range(self.dim())]
    ...
    def elements_satisfying(self, test: Callable[[ndarray], ndarray]) -> ndarray:
        ...
        """
        midp = [np.sum(self.p[itr, self.t], axis=0) / self.t.shape[0] for itr in range(self.dim())]
    ...
    def _expand_facets(self, ix: ndarray) -> Tuple[ndarray, ndarray]:
        ...
        if self.dim() == 3:
            ...
            subset = np.nonzero(np.prod(np.isin(self.edges[:, edge_candidates], self.facets[:, ix].flatten()), axis=0))[0]
    ...
    def _mapping(self):
        ...
        if not hasattr(self, '_cached_mapping'):
            if self.affine:
                ...
                fakemesh = FakeMesh(self.doflocs, self.dofs.element_dofs, self.facets, self.t2f, self.f2t, lambda : self.dim())
                self._cached_mapping = MappingIsoparametric(fakemesh, self.elem(), self.bndelem)
    ...
    def __post_init__(self):
        ...
        if self.nnodes > M:
            ...
            doflocs = np.hstack((p[:, uniq], np.zeros((p.shape[0], np.max(t) + 1 - len(uniq)))))
    ...
    def __repr__(self):
        return '{} mesh with {} vertices and {} elements.'.format(self.elem.refdom.name, self.nvertices, self.nelements)

    def __str__(self):
        return self.__repr__()

    def save(self, filename: str, point_data: Optional[Dict[str, ndarray]]=None, cell_data: Optional[Dict[str, ndarray]]=None, **kwargs) -> None:
    ...
    def from_dict(cls, data):
        ...
        if 'boundaries' in data and data['boundaries'] is not None:
            data['boundaries'] = {k: np.array(v) for (k, v) in data['boundaries'].items()}
        if 'subdomains' in data and data['subdomains'] is not None:
            data['subdomains'] = {k: np.array(v) for (k, v) in data['subdomains'].items()}
    ...
    def to_dict(self) -> Dict[str, Optional[Dict[str, List[float]]]]:
        ...
        return {'p': self.p.T.tolist(), 't': self.t.T.tolist(), 'boundaries': boundaries, 'subdomains': subdomains}
    ...
    def from_mesh(cls, mesh):
        ...
        return cls(doflocs=doflocs, t=mesh.t)
    ...
    def scaled(self, factors):
        ...
        return replace(self, doflocs=np.array([self.doflocs[itr] * factors[itr] for itr in range(len(factors))]))
    ...
    def translated(self, diffs):
        ...
        """
        return replace(self, doflocs=np.array([self.doflocs[itr] + diffs[itr] for itr in range(len(diffs))]))

    def mirrored(self, normal: Tuple[float, ...], point: Optional[Tuple[float, ...]]=None):
    ...
    def build_entities(t, indices, sort=True):
        ...
        (sorted_indexing, ixa, ixb) = np.unique(sorted_indexing, axis=1, return_index=True, return_inverse=True)
