...
class ElementGlobal(Element):
    ...
    def gbasis(self, mapping, X, i, tind=None):
        ...
        if self.V is None:
            self._pbasis_init(self.maxdeg, self.dim, self.derivatives, self.tensorial_basis)
            self.V = np.linalg.inv(self._eval_dofs(mapping.mesh))
        V = self.V[tind]
        x = mapping.F(X, tind=tind)
        U = [np.zeros((self.dim,) * k + x[0].shape) for k in range(self.derivatives + 1)]
        N = len(self._pbasis[()])
        for k in range(self.derivatives + 1):
            diffs = list(itertools.product(*(list(range(self.dim)),) * k))
            for itr in range(N):
                for diff in diffs:
                    U[k][diff] += V[:, itr, i][:, None] * self._pbasis[diff][itr](*x)
        hod = {}
        for k in range(self.derivatives - 2):
            hod['grad{}'.format(k + 3)] = U[k + 3]
        return (DiscreteField(value=U[0], grad=U[1], hess=U[2], **hod),)
        ...
        if j is None and k is None:
            ...
            if dx > 0:
                for l in np.arange(dx, 0, -1):
                    cx *= i - dx + l
            return eval('lambda x: {}*x**{}'.format(cx, np.max([i - dx, 0])))
        elif k is None:
            cx = 1
            cy = 1
            if dx > 0:
            ...
            if dz > 0:
                for l in np.arange(dz, 0, -1):
                    cz *= k - dz + l
            return eval('lambda x, y, z: {}*x**{}*y**{}*z**{}'.format(cx * cy * cz, np.max([i - dx, 0]), np.max([j - dy, 0]), np.max([k - dz, 0])))
    ...
    def _pbasis_init(self, maxdeg, dim, Ndiff, is_tensorial=False):
        ...
        for k in range(Ndiff + 1):
            ...
            for diff in diffs:
                ...
                if dim == 1:
                    self._pbasis[diff] = [self._pbasis_create(i=i, dx=dx) for i in range(maxdeg + 1) if i <= maxdeg]
                elif dim == 2:
                    self._pbasis[diff] = [self._pbasis_create(i=i, j=j, dx=dx, dy=dy) for i in range(maxdeg + 1) for j in range(maxdeg + 1) if is_tensorial or i + j <= maxdeg]
                elif dim == 3:
                    self._pbasis[diff] = [self._pbasis_create(i=i, j=j, k=k, dx=dx, dy=dy, dz=dz) for i in range(maxdeg + 1) for j in range(maxdeg + 1) for k in range(maxdeg + 1) if is_tensorial or i + j + k <= maxdeg]
    ...
    def _eval_dofs(self, mesh, tind=None):
        ...
        w = {'v': np.array([mesh.p[:, mesh.t[itr, tind]] for itr in range(mesh.t.shape[0])])}
        if mesh.p.shape[0] >= 2:
            w['e'] = np.array([0.5 * (w['v'][itr] + w['v'][(itr + 1) % mesh.t.shape[0]]) for itr in range(mesh.t.shape[0])])
            w['n'] = np.array([w['v'][itr] - w['v'][(itr + 1) % mesh.t.shape[0]] for itr in range(mesh.t.shape[0])])
            w['n'][2] = -w['n'][2]
            for itr in range(3):
                w['n'][itr] = np.array([w['n'][itr, 1, :], -w['n'][itr, 0, :]])
