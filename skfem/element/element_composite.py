...
class ElementComposite(Element):
    ...
    def __init__(self, *elems: Element):
        ...
        for (i, e) in enumerate(self.elems):
            for j in range(e.nodal_dofs, e.nodal_dofs + e.edge_dofs):
                dofnames.append(e.dofnames[j] + '^' + str(i + 1))
        for (i, e) in enumerate(self.elems):
            for j in range(e.nodal_dofs + e.edge_dofs, e.nodal_dofs + e.edge_dofs + e.facet_dofs):
                dofnames.append(e.dofnames[j] + '^' + str(i + 1))
        for (i, e) in enumerate(self.elems):
            for j in range(e.nodal_dofs + e.edge_dofs + e.facet_dofs, e.nodal_dofs + e.edge_dofs + e.facet_dofs + e.interior_dofs):
                dofnames.append(e.dofnames[j] + '^' + str(i + 1))
        self.dofnames = dofnames
        doflocs = []
        for i in np.arange(np.sum(np.array([e._bfun_counts() for e in self.elems])), dtype=int):
    ...
    def _deduce_bfun(self, i: int):
        ...
        counts = np.sum(np.array([e._bfun_counts() for e in self.elems]), axis=0)
        tmp: List[Any] = []
        ns: List[Any] = []
        if counts[0] > 0:
            tmp = sum([[j] * self.elems[j].nodal_dofs for j in range(len(self.elems))], [])
            ns += sum([tmp for j in range(int(counts[0] / len(tmp)))], [])
        if counts[1] > 0:
            tmp = sum([[j] * self.elems[j].edge_dofs for j in range(len(self.elems))], [])
            ns += sum([tmp for j in range(int(counts[1] / len(tmp)))], [])
        if counts[2] > 0:
            tmp = sum([[j] * self.elems[j].facet_dofs for j in range(len(self.elems))], [])
            ns += sum([tmp for j in range(int(counts[2] / len(tmp)))], [])
        if counts[3] > 0:
            tmp = sum([[j] * self.elems[j].interior_dofs for j in range(len(self.elems))], [])
