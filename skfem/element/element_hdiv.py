...
class ElementHdiv(Element):
    ...
    def orient(self, mapping, i, tind=None):
        ...
        return -1 + 2 * (mapping.mesh.f2t[0, mapping.mesh.t2f[i]] == np.arange(mapping.mesh.t.shape[1]))
    ...
    def gbasis(self, mapping, X, i, tind=None):
        ...
        return (DiscreteField(value=np.einsum('ijkl,jl,kl->ikl', DF, phi, 1.0 / np.abs(detDF) * orient[:, None]), div=dphi / (np.abs(detDF) * orient[:, None])),)
